...
export default class Puffle extends BaseContainer {
    constructor(puffle, room) {
        ...
        this.color = this.shell.crumbs.puffles[this.color].name
    ...
    loadPuffle() {
        if (this.shell.client.penguin.puffle == this.id) return
        ...
        if (this.shell.client.penguin.id == this.room.id) {
            this.addPuffleInput()
        }
    ...
    onPuffleClick() {
        ...
        this.airtower.sendXt('p#phg', this.id)
    }

    puffleAi() {
        // tells the puffle how to manage the screen position for each possible walking direction
        this.directions = [['s', -0, 1], ['sw', -1, 1][('w', -1, 0)], ['nw', -1, -1], ['n', 0, -1], ['ne', 1, -1], ['e', 1, 0], ['se', 1, 1]]
    ...
    movePuffle(x, y) {
        ...
        if (this.shell.interface.main.puffleCare.visible && this.shell.interface.main.puffleCare.args.puffleId == this.id) {
