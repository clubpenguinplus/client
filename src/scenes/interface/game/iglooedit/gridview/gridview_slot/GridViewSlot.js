...
export default class GridViewSlot extends Phaser.GameObjects.Sprite {
    ...
    onClick() {
        if (!this.shell.room.isIgloo || this.shell.room.id != this.shell.client.id) return

        this.gridView.visible = false
        this.scene.furniture.visible = false

        if (this.filter == 'igloo') {
            this.shell.room.updateIgloo(this.item.id)
        } else {
            this.shell.room.loadFurniture(this.item.id)
        }
    }
    ...
    addIcon(key, item) {
        ...
        if (this.filter != 'igloo') {
            let quantity = this.shell.room.getQuantity(item)
            return this.setQuantity(quantity)
        }

        // Set igloo enable
        this.setEnable(this.shell.room.args.type != item)
    }
